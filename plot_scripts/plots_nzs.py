...
for fieldname in ['GAMA09H','GAMA15H','HECTOMAP','VVDS','WIDE12H','XMMLSS']:
    mskd=fits.open("/global/cscratch1/sd/damonge/HSC_ceci/WIDE_"+
                   fieldname+
                   "_sirius_i24p5_out/masked_fraction.fits")[0].data
    mskbin=np.zeros_like(mskd); mskbin[mskd>0.5]=1
    fng=fits.open("/global/cscratch1/sd/damonge/HSC_ceci/WIDE_"+
                  fieldname+
...
def plot_in_ax(ax,code):
    ...
    for iz in range(4):
        ...
        ax.step(zfs,pz,c=colz[iz])
        pz=nzs_total[code][iz]/np.sum(nzs_total[code][iz])
        ymax=max(ymax,np.amax(pz))
        ax.plot(zs,pz,'-',c=colz[iz])
...
axes[0][0].set_ylabel('$p(z)$',fontsize=14)
axes[1][0].set_ylabel('$p(z)$',fontsize=14)
axes[1][0].set_xlabel('$z$',fontsize=14)
axes[1][1].set_xlabel('$z$',fontsize=14)

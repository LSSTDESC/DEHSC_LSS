...
def plot_ndata(x,y,ey,ax,xcut=None):
    ...
    # Plot data
    ax.errorbar(x,y,yerr=ey,fmt='o',c=formatting.cmap1[5],
                markersize=5, elinewidth=1.5, capthick=1.5, capsize=3.5)

    # Plot linear fit
    x0=x[0]-(x[-1]-x[0])*0.05
    xf=x[-1]+(x[-1]-x[0])*0.05
    ax.plot([x0,xf],[m*(x0-1)+1,m*(xf-1)+1],'k-',lw=2)
    # Plot cutoff
    if xcut:
        y0=np.amin(y-ey)
        yf=np.amax(y+ey)
        
        ax.plot([xcut,xcut],[y0-0.05*(yf-y0),yf+0.05*(yf-y0)],'--',c='#AAAAAA',lw=2)
    ...
    ax.set_ylabel('$R(N_g)$',fontsize=18)
...
plot_ndata(d_airmass['x_rescaled'][1],
           ...
           d_airmass['error'][1],
           axes[0],xcut=1.15)
axes[0].set_xlabel('$R({\\rm Airmass},r)$',fontsize=18)
# Plot dust
plot_ndata(d_dust['x_rescaled'][0],
           d_dust['mean'][0],
           d_dust['error'][0],
           axes[1])
axes[1].set_xlabel('$R({\\rm Dust})$',fontsize=18)

...
\end{figure}
    ...
    In our second test, we compare the constraints derived using the redshift distributions obtained from  COSMOS30 to those obtained with the photometric redshift codes \texttt{DEmP}, \texttt{Ephor}, \texttt{Ephor\_AB} and \texttt{FRANKEN-Z} \cite{2018PASJ...70S...9T}. We first parameterize systematic uncertainties in these distributions by only accounting for shifts in the means $\Delta z_{i}$. The comparison of the constraints obtained from all five methods is shown in Fig.~\ref{fig:constraints-fit-pz-shifts-pz-methods}. As can be seen, the values for the $\Delta z_{i}$ required by the different codes are significantly different, which leads to differences in both constraining power and constraints on the HOD parameters. Investigating this further, we find that the observed differences in $\Delta z_{i}$ cannot be explained with the differences in mean redshift between the distributions derived using the five separate methods. In addition, we find that shifts in the mean of the redshift distributions mainly affect low redshift auto power spectra but have a significantly smaller impact on our high redshift bins. This suggests that our photo-$z$ error model is too simplistic to capture the differences between the distributions and we therefore compare the constraints from the different codes additionally accounting for uncertainties in the widths of the distributions. As can be seen from Fig.~\ref{fig:constraints-fit-pz-shifts+pz-widths-pz-methods}, accounting for both shifts in the means and changes in the widths brings the constraints from all codes in quite good agreement. In particular, the constraints on HOD parameters now agree very well. The only exception is a discrepancy between the parameters $z_{w, 3}$ and $\Delta z_{3} - \Delta z_{2}$. To better understand this remaining discrepancy, we compare the auto power spectra obtained when forcing the five redshift distributions to have approximately the same means and widths. We find that the resulting power spectra still exhibit significant differences, which suggests that features in the redshift distribution beyond mean and width matter for galaxy clustering statistics. In our case, accounting for uncertainties in means and widths brings the constraints on HOD parameters in good agreement and we therefore conclude that this error model is accurate enough to characterize photo-$z$ uncertainties in the present case. However, this will probably cease to be true for future photometric clustering analyses (e.g. using LSST data) and suggests that these data sets will have to be analyzed accounting for the full uncertainty on the shape of the photometric redshift distributions.

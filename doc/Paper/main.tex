...
\subsection{Photometric redshift systematics modeling}\label{ssec:theory.photoz_syst}
    ...
    \begin{table*}
      ...
      \begin{center}
        \begin{tabular}{cccc}
          ...
          $M_{\mathrm{min}}$ & flat $\in [0., \,17.]$ & $12.29 \pm 0.15$ & $-$ \\ 
          $M_{\mathrm{min}, p}$ & flat $\in [-5., \,10.]$ & $2.93\substack{+1.56 \\ -1.47}$ & $-$ \\
          $M_{0}$ & flat $\in [0., \,15.]$ & $5.97 \pm 4.00$ & $-$ \\
          $M_{0, p}$ & flat $\in [-5., \,10.]$ & $2.65 \pm 5.00$ & $-$ \\
          $M_{1}$ & flat $\in [0., \,15.]$ & $13.69 \pm 0.21$ & $-$ \\ 
          $M_{1, p}$ & flat $\in [-5., \,15.]$ & $6.00\substack{+2.14 \\ -2.05}$ & $-$ \\
          $\Delta z_{0}$ & flat $\in [-0.2, \,0.2]$ & $0.0170\substack{+0.091 \\ -0.090}$ & $-$ \\
          $\Delta z_{1}$ & flat $\in [-0.2, \,0.2]$ & $0.00470\substack{+0.095 \\ -0.093}$ & $-$ \\
          $\Delta z_{2}$ & flat $\in [-0.2, \,0.2]$ & $0.0198\substack{+0.101 \\ -0.099}$ & $-$ \\
          $\Delta z_{3}$ & flat $\in [-0.2, \,0.2]$ & $0.0337\substack{+0.106 \\ -0.105}$ & $-$ \\
          $z_{w, 0}$ & flat $\in [-0.2, \,0.2]$ & $-0.0361\substack{+0.1376 \\ -0.1234}$ & $-$ \\
          $z_{w, 1}$ & flat $\in [-0.2, \,0.2]$ & $0.0252 \pm 0.0799$ & $-$ \\
          $z_{w, 2}$ & flat $\in [-0.2, \,0.2]$ & $0.0569\substack{+0.0856 \\ -0.0897}$ & $-$ \\
          $z_{w, 3}$ & flat $\in [-0.2, \,0.2]$ & $0.0153\substack{+0.1190 \\ -0.1217}$ & $-$ \\ \\
          $A_{\mu}$ & flat $\in [-2., \,2.]$ & $ - $ & $ - $ \\
...
In order to test the robustness of our fiducial HOD constraints to implementation and data choices, we compare the constraints from several analysis variants, as described in the following.

\begin{table*}
\caption{Parameter constraints and best-fit $\chi^{2}$ values obtained for all considered analysis variants.} \label{tab:constraints_robustness}
\begin{center}
\begin{tabular}{ccccccccc}
\hline\hline 
Analysis variant & $\sfrac{\chi^{2}}{\nu}$ & $M_{\mathrm{min}, p}$ & $M_{\mathrm{min}}$ & $M_{1, p}$ & $M_{1}$ & $A_{\mu}$ & $\Omega_{c}$ & $\sigma_{8}$ \\ \hline \Tstrut       
fiducial & $\sfrac{95.8}{80}$ & $2.93\substack{+1.56 \\ -1.47}$ & $12.29 \pm 0.15$ & $6.00\substack{+2.14 \\ -2.05}$ & $13.69 \pm 0.21$ & - & - & - \\
auto & - & - & - & - & - & - & - & - \\
G cov & - & - & - & - & - & - & - & - \\
G+SSC cov & - & - & - & - & - & - & - & - \\
no $z_{w, i}$ & - & - & - & - & - & - & - & - \\
no $z_{w, i}, \Delta z_{i}$ & - & - & - & - & - & - & - & - \\
...
As discussed above, our fiducial constraints are derived from a joint fit to both auto- and cross power spectra. However, cross correlations between redshift bins are susceptible to photometric redshift errors, such as outliers. In order to test for systematics affecting the cross-correlations, we therefore test the consistency of the auto- and cross power spectra by comparing the constraints obtained using only auto power spectra to those obtained when jointly fitting auto- and cross power spectra. Fig.~\ref{fig:constraints-cov=G+NG+SSC-vs-cov=G-vs-cov=G+SSC-vs-fit=auto} shows the comparison between the HOD constraints obtained from auto power spectra to those obtained from auto- and cross power spectra. As can be seen, the constraints from auto power spectra and auto- and cross power spectra agree very well with each other, suggesting that uncertainties in photometric redshifts do not significantly affect the cross correlations measured in our analysis. In addition we see that the constraining power on HOD parameters is unaffected by including the cross power spectra. \todo{chi2 from auto theory to auto+cross theory}

An important part of our data model is the analytical covariance matrix described in Sec.~\ref{ssec:methods.covar}. In order to test the impact of non-Gaussian contributions to the covariance on our results, we compare the HOD constraints obtained accounting for Gaussian (G), Gaussian and SSC (G+SSC) and Gaussian, SSC and non-Gaussian (G+NG+SSC) contributions. As can be seen from Fig.~\ref{fig:constraints-cov=G+NG+SSC-vs-cov=G-vs-cov=G+SSC-vs-fit=auto}, these constraints agree very well with each other. The constraints from the G and the G+SSC case yield almost identical constraints, which is expected due to the suppression of the SSC contribution by the number density correction (c.f. Sec.~\ref{ssec:results.covariance}). Accounting for all non-Gaussian contributions to the covariance results in slightly broadened but consistent constraints. Comparing to Fig.~\ref{fig:covariance-contributions}, this is probably due to the fact that these corrections only affect the lowest redshift bins at small angular scales and therefore do not have a significant impact when computing constraints from all power spectra.
...
\caption{Comparison of the constraints on photo-$z$ systematics parameters obtained from auto power spectra alone to those obtained from auto and cross power spectra. The inner (outer) contour shows the $68 \%$ c.l. ($95 \%$ c.l.).}
...
As a last test of robustness against photometric redshift errors, we compare the constraints obtained using the COSMOS redshift distributions to those obtained with the photo-$z$ codes \texttt{Ephor\_AB}, \texttt{Ephor}, \texttt{DEmP} and \texttt{FRANKEN-Z}, as provided by the HSC Collaboration \cite{2018PASJ...70S...9T}. In analogy to our fiducial analysis, we use the photometric redshift error model given in Eq.~\ref{eq:photo-z-model} for each photo-$z$ code. The comparison of the constraints on HOD and photo-$z$ systematics parameters obtained for the five different methods are shown in Figures \ref{fig:constraints-HOD-fit-pz-shifts+pz-widths-pz-methods} and \ref{fig:constraints-pz-syst-fit-pz-shifts+pz-widths-pz-methods}. As can be seen, the constraints on HOD parameters from all codes are in very good agreement, while we see some discrepancies between the constraints on photometric redshift systematics parameters, especially between $z_{w, 3}$ and $\Delta z_{3} - \Delta z_{2}$. However, these differences are not unexpected as the different redshift distributions have different means and shapes and are therefore not required to give consistent constraints on photo-$z$ systematics parameters. To better understand the source for these discrepancies, we compare the auto power spectra obtained when forcing the five redshift distributions to have approximately the same means and widths. We find that the resulting power spectra exhibit significant differences, which suggests that features in the redshift distribution beyond mean and width can significantly impact observed galaxy clustering power spectra. In our case, accounting for uncertainties in means and widths gives consistent constraints on HOD parameters and we therefore conclude that this error model is accurate enough to characterize photo-$z$ uncertainties in the present case. However, this will probably cease to be true for future photometric clustering analyses (e.g. using LSST data) and suggests that these data sets will have to be analyzed accounting for the full uncertainty on the shape of the photometric redshift distributions.

In order to further illustrate this last point, we perform two additional analyses in which we compare the constraints obtained from the five photo-$z$ methods setting $z_{w, i} = 0$ and $z_{w, i} = 0, \Delta z_{i} = 0$ respectively. The results are shown in Figures \ref{fig:constraints-fit-pz-shifts-pz-methods} and \todo{bla}. As can be seen, the values for the photo-$z$ systematics parameters $\Sigma_{i} \Delta z_{i}, \Delta z_{i} - \Delta z_{j}$ required by the different codes are significantly different, which leads to differences in both constraining power and constraints on HOD parameters. Investigating this further, we find that the observed differences in $\Delta z_{i}$ cannot be explained with the differences in mean redshift between the distributions derived using the five separate methods. In addition, we find that shifts in the mean of the redshift distributions mainly affect low redshift auto power spectra but have a significantly smaller impact on our high redshift bins. As noted above, this suggests that the power spectra are affected by features beyond the mean of the photo-$z$ distribution and our analysis is sensitive to those, as borne out by the differences in HOD constraints seen in Fig.~\ref{fig:constraints-fit-pz-shifts-pz-methods}. \todo{Repeat analysis without pz shifts}

\begin{figure}
\begin{center}
\includegraphics[width=0.95\textwidth]{figures/contours_mPk=HOD_fix=alpha-fc-sigmaM_HOD=zevol_fit=pz-shifts+prior=0p2_fit=auto+cross_cosmo=const_cov=G+NG+SSC-LINBIAS_HOD-param=zfid_clfit=HOD-zevol_pz-methods.pdf}
\caption{Comparison of the constraints on HOD and systematics parameters obtained using the redshift distributions derived from COSMOS30, \texttt{DEmP}, \texttt{Ephor}, \texttt{Ephor\_AB} and \texttt{FRANKEN-Z} accounting for only shifts in the means of the distributions. The inner (outer) contour shows the $68 \%$ c.l. ($95 \%$ c.l.).}
\label{fig:constraints-fit-pz-shifts-pz-methods}
\end{center}
\end{figure}
...
Using the estimated $s(z)$, we can derive theoretical predictions for the angular galaxy power spectra in presence of magnification. In order to determine the best-fit model parameters accounting for magnification bias we run two different MCMCs: in the first case, we fix the magnification bias amplitude to the predictions from the empirically determined $s(z)$ function. In the second case we include an additional magnification bias parameter $A_{\mu}$ that scales the amplitude of the magnification kernel $W_{\mu}$ and we fit it alongside the other parameters.

From a joint fit to all auto- and cross power spectra we obtain $A_{\mu} = 0.99\substack{+0.37 \\ -0.35}$ and the constraint is shown in Fig.~\ref{fig:constraints-fit=mag-bias-ampl_fit=auto+cross-vs-fit=mag-bias-ampl_fit=auto}. This is consistent with our fiducial model for $s(z)$, which corresponds to $A_{\mu} = 1$ and constitutes a $\sim 2.8 \sigma$ detection of magnification bias from our HSC sample. The best-fit $\chi^{2}$ including magnification is $\chi^{2} = 83.2$\footnote{This is consistent with the case in which we set $A_{\mu}=1$, where we obtain consistent constraints with best-fit $\chi^{2} = 84.1$.}, which, computing the degrees of freedom as $\nu = N_{\mathrm{data}} - N_{\mathrm{param}} = 94 - 15 = 79$, corresponds to $\chi^{2}_{\mathrm{red}} = \sfrac{\chi^{2}}{\nu} = 1.06$ ($p$-value $= 0.35$). This corresponds to an improvement in $\chi^{2}$ compared to the fiducial analysis of $\Delta \chi^{2} = 95.4 - 83.2 = 12.2$, consistent with the $\sim 3 \sigma$ detection. The constraints obtained when including magnification agree very well with our fiducial constraints, as can be seen from Fig.~\ref{fig:constraints-no-mag-bias-vs-fit=mag-bias-ampl}. In addition, we find the significance of the magnification detection to be largely insensitive to photo-$z$ systematics modeling choices, as it is not significantly affected by relaxing our photo-$z$ systematics model by fixing $z_{w, i} = 0$. In order to identify the part of the data vector driving the constraints on $A_{\mu}$ we repeat this analysis only including the auto power spectra. The comparison between the constraints from auto- and cross power spectra to those obtained from auto power spectra alone in shown in Fig.~\ref{fig:constraints-fit=mag-bias-ampl_fit=auto+cross-vs-fit=mag-bias-ampl_fit=auto}. As can be seen, the auto power spectra alone are not able to constrain $A_{\mu}$, which implies that the constraints on the magnification amplitude are solely driven by the cross power spectra. In analogy to the results for the photo-$z$ systematics parameters, we thus find that cross power spectra carry important information and it will thus be beneficial to include these cross correlations in current and future photometric clustering analyses. 
...
\end{figure}
 ...
 \begin{figure}
\begin{center}
\includegraphics[width=0.95\textwidth]{figures/contours_mPk=HOD_fix=alpha-fc-sigmaM_HOD=zevol_fit=pz-shifts-pz-widths+prior=0p2_cosmo=const_cov=G+NG+SSC-LINBIAS_HOD-param=zfid_clfit=HOD-zevol_fit=mag-bias-ampl_fit=auto+cross-vs-fit=auto.pdf}
\caption{Comparison of the constraints on magnification obtained from auto power spectra alone to those obtained from auto and cross power spectra. The inner (outer) contour shows the $68 \%$ c.l. ($95 \%$ c.l.).}
...
As described in Sec.~\ref{ssec:results.hod-constraints}
...
\begin{figure}
\begin{center}
\subfigure[...]{\includegraphics[width=0.3\textwidth]{figures/bias-redshift_mPk=HOD_fix=alpha-fc-sigmaM_HOD=zevol_fit=pz-shifts+prior=0p2-pz-widths+prior=0p2_fit=auto+cross_cosmo=const_cov=G+NG+SSC-LINBIAS_HOD-param=zfid_clfit=HOD-zevol.pdf}}
\subfigure[...]{\includegraphics[width=0.3\textwidth]{figures/bias-growth_mPk=HOD_fix=alpha-fc-sigmaM_HOD=zevol_fit=pz-shifts+prior=0p2-pz-widths+prior=0p2_fit=auto+cross_cosmo=const_cov=G+NG+SSC-LINBIAS_HOD-param=zfid_clfit=HOD-zevol.pdf}} 
\subfigure[Dependence of the mean halo mass $\bar{M}_{\mathrm{halo}}$ on redshift as predicted from our best-fit HOD model.]{\includegraphics[width=0.3\textwidth]{figures/M_halo_mPk=HOD_fix=alpha-fc-sigmaM_HOD=zevol_fit=pz-shifts+prior=0p2-pz-widths+prior=0p2_fit=auto+cross_cosmo=const_cov=G+NG+SSC-LINBIAS_HOD-param=zfid_clfit=HOD-zevol.pdf}} 
\caption{...} 
...
\subsubsection{Joint cosmology and HOD constraints}
...
To illustrate the cosmological constraining power of the data considered in our analysis, we allow for variations in the r.m.s. of linear matter fluctuations in spheres of comoving radius 8 $h^{-1}$ Mpc, $\sigma_{8}$, and the fractional cold matter density today, $\Omega_{c}$, in addition to our baseline set of parameters (c.f. Sec.~\ref{ssec:methods.constr}). All other cosmological parameters remain fixed to their fiducial Planck 2018 values. From a joint fit to all auto- and cross power spectra we obtain $\sigma_{8} = 0.83 \pm 0.20$ and $\Omega_{c} = 0.230 \pm 0.025$, consistent with our assumed fiducial cosmology. The constraints on HOD and photo-$z$ systematics parameters agree well with our fiducial results, which shows that the constraining power of the data on astrophysical and photo-$z$ systematics parameters is not significantly affected by allowing for variations in $\Omega_{c}$ and $\sigma_{8}$. Our exploratory cosmological analysis further shows that the data are able to constrain $\Omega_{c}$ while $\sigma_{8}$ is largely unconstrained, probably owing to the degeneracy with galaxy bias. The constraints on HOD and cosmological parameters are shown in Fig.~\ref{fig:constraints-fit=Oc+s8}.
 
 \begin{figure}
\begin{center}
\includegraphics[width=0.95\textwidth]{figures/contours_mPk=HOD_fix=alpha-fc-sigmaM_HOD=zevol_fit=pz-shifts-pz-widths+prior=0p2_fit=Oc+s8_fit=auto+cross_cov=G+NG+SSC-LINBIAS_HOD-param=zfid_clfit=HOD-zevol.pdf}
\caption{Constraints on HOD and cosmological parameters obtained from a joint fit to all auto- and cross power spectra. The inner (outer) contour shows the $68 \%$ c.l. ($95 \%$ c.l.).}

...
\end{equation}  
where we have assumed the bias to be separable in $z$ and $m_{\mathrm{lim}}$ and the parameter $\alpha$ accounts for deviations to the simple inverse growth function proportionality.
...
\end{aligned}  
\end{equation}
where $b_{1} = -0.0631$ and $b_{0} = 2.35$. \as{Do we want to rewrite this as $\bar{b} = b_0 + b_1 (m-24)$?} The results from this fitting function are shown as solid lines in Fig.~\ref{fig:magnitude-cuts-bias}, alongside the values obtained from the HOD fit.
...
We anticipate that these results will mostly be useful for forecasting the  constraining power of magnitude-limited samples in Fisher matrix forecasts. \as{Do we want to show our result for LSST vs SRD assumption of $0.95/D(z)$?}

An alternative use might be galaxy evolution studies, where these results provide a convenient cross-check formula. Our full parameter chains for the different limiting magnitudes are available upon request. \as{Is this the case?} \an{That would be totally fine with me.}

...
\section{Methods}\label{sec:methods}
  ...
  \subsection{Systematics maps}\label{ssec:methods.syst}
    ...
    \begin{enumerate}
      \item {\bf Dust extinction}: we make maps of dust absorption in each band using the data from \todo{cite} projected onto the 6 HSC DR1 fields.
  ...
  \subsection{Redshift distributions}\label{ssec:methods.nz}
    ...
    To estimate the fiducial redshift distributions used in our main analysis we make use of the COSMOS 30-band photometric catalog of \todo{cite}. The idea is to use the high-fidelity photo-$z$ estimates in the COSMOS 30-band data as the truth, in which case one can estimate the redshift distribution by simply histogramming these redshifts with appropriate weights. These weights are calculated following the procedure described in \todo{cite}, which we summarize here for completeness:
    \begin{enumerate}

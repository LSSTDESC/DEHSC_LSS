...
\hline\hline 
Analysis variant & $\sfrac{\chi^{2}}{\nu}$ & $M_{\mathrm{min}, p}$ & $M_{\mathrm{min}}$ & $M_{1, p}$ & $M_{1}$ & $A_{\mu}$ & $\Omega_{c}$ & $\sigma_{8}$ \\ \hline      
fiducial & - & - & - & - & - & - & - & - \\
auto & - & - & - & - & - & - & - & - \\
G cov & - & - & - & - & - & - & - & - \\
G+SSC cov & - & - & - & - & - & - & - & - \\
...
It is instructive to investigate which part of the data are driving the constraints on photometric redshift systematics. To this end we compare the constraints obtained using auto power spectra only to those obtained from auto- and cross power spectra. As can be seen from Fig.~\ref{fig:constraints-auto-vs-auto+cross}, we find that the auto power spectra do not constrain the photometric redshift systematics parameters $\Delta z_{i} - \Delta z_{j}$, as opposed to the combination of auto- and cross power spectra. This shows that the cross correlations drive the constraints on photometric redshift systematics and are thus essential for jointly constraining redshift systematics and astrophysics/cosmological parameters from LSS.
...
\end{figure}
 ...
 \begin{figure}
\begin{center}
\includegraphics[width=0.95\textwidth]{figures/contours_mPk=HOD_fix=alpha-fc-sigmaM_HOD=zevol_fit=pz-shifts+prior=0p2-pz-widths+prior=0p2_fit=auto+cross_cosmo=const_cov=G+NG+SSC-LINBIAS_HOD-param=zfid_clfit=HOD-zevol_pz-methods.pdf}
\caption{Comparison of the constraints on HOD and systematics parameters obtained using the redshift distributions derived from COSMOS30, \texttt{DEmP}, \texttt{Ephor}, \texttt{Ephor\_AB} and \texttt{FRANKEN-Z} accounting for both shifts in the means and changes in the widths of the distributions. The inner (outer) contour shows the $68 \%$ c.l. ($95 \%$ c.l.).}
\label{fig:constraints-fit-pz-shifts+pz-widths-pz-methods}
\end{center}
\end{figure}
...
From a joint fit to all auto- and cross power spectra we obtain $A_{\mu} = 0.99\substack{+0.37 \\ -0.35}$ and the constraints are shown in Fig.~\ref{fig:constraints-fit=mag-bias-ampl}. This is consistent with our model for $s(z)$, which corresponds to $A_{\mu} = 1$ and constitutes a $\sim 2.8 \sigma$ detection of magnification bias from our HSC sample. The best-fit $\chi^{2}$ including magnification is $\chi^{2} = 83.2$\footnote{This is consistent with the case in which we set $A_{\mu}=1$, where we obtain consistent constraints with best-fit $\chi^{2} = 84.1$.}, which, computing the degrees of freedom as $\nu = N_{\mathrm{data}} - N_{\mathrm{param}} = 94 - 15 = 79$, corresponds to $\chi^{2}_{\mathrm{red}} = \sfrac{\chi^{2}}{\nu} = 1.06$ ($p$-value $= 0.35$). This corresponds to an improvement in $\chi^{2}$ compared to the fiducial analysis of $\Delta \chi^{2} = 95.4 - 83.2 = 12.2$, consistent with the $\sim 3 \sigma$ detection. The constraints obtained when including magnification agree very well with our fiducial constraints, as can be seen from Fig.~\ref{fig:constraints-no-mag-bias-vs-fit=mag-bias-ampl}. In addition, we find the significance of the magnification detection to be insensitive to photometric redshift systematics, as it is unchanged when we account for changes in the width of the photometric redshift distributions in addition to shifts in the mean. In order to investigate what part of the data vector is driving the constraints on $A_{\mu}$ we repeat this analysis only including the auto power spectra. The comparison between the constraints from auto- and cross power spectra to those from auto power spectra only in shown in Fig.~\ref{fig:constraints-fit=mag-bias-ampl_fit=auto+cross-vs-fit=auto}. As can be seen, the auto power spectra alone are not able to constrain $A_{\mu}$, which means that the constraints on the magnification amplitude are solely driven by the cross power spectra.

 \begin{figure}
\begin{center}
\includegraphics[width=0.95\textwidth]{figures/contours_mPk=HOD_fix=alpha-fc-sigmaM_HOD=zevol_fit=pz-shifts-pz-widths+prior=0p2_fit=auto+cross_cosmo=const_cov=G+NG+SSC-LINBIAS_HOD-param=zfid_clfit=HOD-zevol_fit=mag-bias-ampl.pdf}
\caption{Parameter constraints obtained when allowing for a free amplitude of the magnification kernel $A_{\mu}$. The inner (outer) contour shows the $68 \%$ c.l. ($95 \%$ c.l.).}
\label{fig:constraints-fit=mag-bias-ampl}
\end{center}
\end{figure}

 \begin{figure}
\begin{center}
\includegraphics[width=0.95\textwidth]{figures/contours_mPk=HOD_fix=alpha-fc-sigmaM_HOD=zevol_fit=pz-shifts-pz-widths+prior=0p2_fit=auto+cross_cosmo=const_cov=G+NG+SSC-LINBIAS_HOD-param=zfid_clfit=HOD-zevol_no-mag-bias-vs-fit=mag-bias-ampl.pdf}
\caption{Comparison of the constraints on HOD parameters when accounting for magnification to the fiducial constraints. The inner (outer) contour shows the $68 \%$ c.l. ($95 \%$ c.l.).}
...
\caption{Comparison of the constraints on HOD parameters and magnification obtained from auto power spectra alone to those obtained from auto and cross power spectra. The inner (outer) contour shows the $68 \%$ c.l. ($95 \%$ c.l.).}
\label{fig:constraints-fit=mag-bias-ampl_fit=auto+cross-vs-fit=auto}
\end{center}
\end{figure}

...
From a joint fit to all auto- and cross power spectra we obtain $A_{\mu} = 0.99\substack{+0.37 \\ -0.35}$ and the constraints are shown in Fig.~\ref{fig:constraints-fit=mag-bias-ampl}. This is consistent with our model for $s(z)$, which corresponds to $A_{\mu} = 1$ and constitutes a $\sim 2.8 \sigma$ detection of magnification bias from our HSC sample. The best-fit $\chi^{2}$ including magnification is $\chi^{2} = 83.2$, which corresponds to an improvement compared to the fiducial analysis of $\Delta \chi^{2} = 95.4 - 83.2 = 12.2$, consistent with the $\sim 3 \sigma$ detection. The constraints obtained when including magnification agree very well with our fiducial constrains, as can be seen from Fig.~\ref{fig:constraints-no-mag-bias-vs-fit=mag-bias-ampl}. In addition, we find the significance of the magnification detection to be insensitive to photometric redshift systematics, as it is unchanged when we account for changes in the width of the photometric redshift distributions in addition to shifts in the mean. In order to investigate what part of the data vector is driving the constraints on $A_{\mu}$ we repeat this analysis only including the auto power spectra. The comparison between the constraints from auto- and cross power spectra to those from auto power spectra only in shown in Fig.~\ref{fig:constraints-fit=mag-bias-ampl_fit=auto+cross-vs-fit=auto}. As can be seen, the auto power spectra alone are not able to constrain $A_{\mu}$, which means that the constraints on the magnification amplitude are solely driven by the cross power spectra.

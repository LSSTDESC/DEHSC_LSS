...
Using the estimated $s(z)$, we can derive theoretical predictions for the angular galaxy power spectra in presence of magnification. In order to determine the best-fit model parameters accounting for magnification we run two different MCMCs: in the first case, we fix the magnification bias amplitude to the predictions from the empirically determined $s(z)$ function. In the second case we include an additional magnification parameter $A_{\mu}$ that scales the amplitude of the magnification kernel $W_{\mu}$ and we fit it alongside the other parameters.
...
\an{As described in Sec.~\ref{ssec:results.hod-constraints}}
...
To illustrate the cosmological constraining power of the data considered in our analysis, we allow for variations in the r.m.s. of linear matter fluctuations in spheres of comoving radius 8 $h^{-1}$ Mpc, $\sigma_{8}$, and the fractional cold matter density today, $\Omega_{c}$, in addition to our baseline set of parameters (c.f. Sec.~\ref{ssec:methods.constr}). All other cosmological parameters remain fixed to their fiducial Planck 2018 values. From a joint fit to all auto- and cross power spectra we obtain $\sigma_{8} = 0.837\substack{+0.193 \\ -0.211}$ and $\Omega_{c} = 0.228\substack{+0.022 \\ -0.023}$, consistent with our assumed fiducial cosmology, and the constraints are shown in Fig.~\ref{fig:constraints-fit=Oc+s8}. As can be seen from Tab.~\ref{tab:tab:constraints_robustness}, the derived constraints on HOD parameters agree well with our fiducial results, which shows that the constraining power of the data on astrophysical parameters is not significantly affected by allowing for variations in $\Omega_{c}$ and $\sigma_{8}$. Our exploratory cosmological analysis further shows that the data are able to constrain $\Omega_{c}$ while $\sigma_{8}$ is largely unconstrained, probably owing to the degeneracy with galaxy bias. \an{More discussion here. Restructure!}

...
One of the most important potential systematics in photometric galaxy clustering analyses are photometric redshift uncertainties and we therefore test the stability of our results to photo-$z$ in several different ways. In Fig.~\todo{bla} we show the constraints on photometric redshift systematics parameters derived in our fiducial analysis. We find that the data cannot separately constrain the mean shift parameters $\Delta z_{i}$ as opposed to pairwise differences between those. We therefore show the constraints in terms of the reparametrized parameters $\Sigma \Delta z_{i}, \Delta z_{1} - \Delta z_{0}, \Delta z_{2} - \Delta z_{1}, \Delta z_{3} - \Delta z_{2}$, which denote the sum of the $\delta z_{i}$ and their pairwise differences respectively. 

\todo{move this to photo-z section} As expected, we find that the auto power spectra alone yield generally weaker constraints than the combination of auto- and cross power spectra. As can be seen, the auto power spectra alone cannot constrain the photometric redshift systematics parameters $\Delta z_{i}$, suggesting that a significant part of the constraining power in our analysis stems from the cross correlations.

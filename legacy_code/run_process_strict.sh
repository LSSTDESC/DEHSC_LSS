...
nz_method=zmc #Currently available: zmc, pdfstack, cosmos30

#First clean up the metadata
for table in WIDE DEEP UDEEP
do
    if [ "$do_cleanup" = true ]; then
	${python_exec} process_metadata.py --input-file ${predir_out}/HSC_${table}_frames.fits --output-file ${predir_out}/HSC_processed_strict/HSC_${table}_frames_proc.fits
    fi
done

#Now clean up the WIDE
for field in WIDE_AEGIS WIDE_GAMA09H WIDE_GAMA15H WIDE_HECTOMAP WIDE_VVDS WIDE_WIDE12H WIDE_XMMLSS
do
    dirname=${predir_out}/HSC_processed_strict/${field}
    mkdir -p ${dirname}
    if [ "$do_process" = true ]; then
	echo $field
	${python_exec} process.py --input-field ${field} --resolution 0.01 --field-padding 0.1 --output-prefix ${dirname}/${field} --save-systematics --save-masks --save-depth-maps --min-snr 10.0 --depth-cut 24.5 --flat-project CAR --strict-cuts
    fi
done
...
do
	...
	echo $field
	dirname=${predir_out}/HSC_processed_strict/${field}
	${python_exec} map_obscond.py --input-frames ${predir_out}/HSC_processed_strict/HSC_WIDE_frames_proc.fits --map-sample ${dirname}/${field}_MaskedFraction.fits --output-prefix ${dirname}/${field}
    fi
done
...
#Run diagnostics 
for field in WIDE_GAMA15H WIDE_HECTOMAP WIDE_VVDS WIDE_WIDE12H WIDE_XMMLSS
do

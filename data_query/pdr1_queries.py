...
def write_fieldsearch_meas(tablename,fieldname,fname_out,output_format="fits",
                           do_field=True,submit=False,ra_range=None,
                           do_photoz=False,strict_cuts=False,do_download=True,part=None) :
    filters=['g','r','i','z','y']
    stout="-- Run field, "+fname_out+"\n"
    ...
    def add_filters(name,behind=True,meas=False) :
        sthere=""
        for fi in filters :
            if behind :
                if meas:
                    sthere+="       ,meas."+name+fi+" as "+name+fi+"_m\n"
                else:
                    sthere+="       ,forced."+name+fi+"\n"
            else :
                if meas:
                    sthere+="       ,meas."+fi+name+" as "+fi+name+"_m\n"
                else:
                    sthere+="       ,forced."+fi+name+"\n"
        return sthere
    ...
    stout+=add_filters("cmodel_mag",behind=False)
    stout+=add_filters("cmodel_mag_err",behind=False)
    stout+=add_filters("cmodel_flux",behind=False,meas=True)
    stout+=add_filters("cmodel_flux_err",behind=False,meas=True)
    stout+=add_filters("cmodel_flux_flags",behind=False,meas=True)
    stout+=add_filters("cmodel_mag",behind=False,meas=True)
    stout+=add_filters("cmodel_mag_err",behind=False,meas=True)
    if do_photoz :
        stout+=add_photoz("eab")
        stout+=add_photoz("frz")
        stout+=add_photoz("nnz")
